...
public class TrackCommand implements Command, Reply {
    ...
    public boolean isTriggered(Update update) {
        return Command.super.isTriggered(update) || isReply(update, BotReply.TRACK_COMMAND.getReply());
    }

    @Override
    public SendMessage handle(Update update) {
        if (Command.super.isTriggered(update)) {
            return BotSendMessage.getSendMessage(update.message().chat().id(), BotReply.TRACK_COMMAND.getReply(), true);
        }
    ...
    public SendMessage handleReply(Update update) {
        return linkHandler.handleLink(update, scrapperService::addLink, BotReply.LINK_ADDED.getReply());
    }
}
